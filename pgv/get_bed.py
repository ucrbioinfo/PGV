...
def get_bed(consensus, node_orders, node_lengths, chrm_end_nodes, chrm_end_c_nodes, input_genomes, num_of_chrms, bed_aligned):
	...
	for i in range(len(consensus_whead)):
		...
		if curr_node.startswith("C"):
			...
			if chrm_count <= num_of_chrms:
				...
				chrmName = "contigs"
			print(chrmName, start_pos, endPos, curr_node, 1, "+", start_pos, endPos,  colorConsensus, sep='\t', file=OUTconsensus)
	...
	for i in range(num_of_genomes):
		...
		for j in range(len(node_order_whead)):
			...
			if curr_node.startswith("C"):
				...
				if neighbor_cs_w_direction[curr_node] == [all_c_nodes[i][Cindex-1], all_c_nodes[i][Cindex+1]]: ## same prev and next
					print(chrmName, start_pos, endPos, curr_node, 1, "+", start_pos, endPos, colorC, sep='\t', file=outFile)
				elif neighbor_cs_no_direction[curr_node] == set([all_c_nodes[i][Cindex-1], all_c_nodes[i][Cindex+1]]): ## rev
					print(chrmName, start_pos, endPos, curr_node, 1, "-", start_pos, endPos, colorCrev, sep='\t', file=outFile)
				else: ## translocation
					print(chrmName, start_pos, endPos, curr_node, 1, "+", start_pos, endPos, colorCtrans, sep='\t', file=outFile)
				start_pos = endPos + 1

			elif curr_node.startswith("D"):
				endPos = start_pos + node_lengths[curr_node][i]
				print(chrmName, start_pos, endPos, curr_node, 1, "+", start_pos, endPos, colorP, sep='\t', file=outFile)
				start_pos = endPos + 1
			elif curr_node.startswith("U"):
				endPos = start_pos + node_lengths[curr_node][i]
				print(chrmName, start_pos, endPos, curr_node, 1, "+", start_pos, endPos, colorU, sep='\t', file=outFile)
				start_pos = endPos + 1
			else: ## dummy/Tail
				chrm_count += 1
				start_pos = 1
